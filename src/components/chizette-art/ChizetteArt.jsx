    ...
    <ParallaxProvider className="App container">
      <AdminProvider>
        ...
        {filteredTerm === 'Splash' ? (
          ...
          <i>
            <h4 className="filteredTitle">{filteredTerm}</h4>
          </i>
        )}
        {secretLogIn && <LoginForm />}
        <SnackbarProvider>
          <CrudProvider>
            {filteredTerm === 'Splash' && (
              <SplashList configureFilteredTerm={configureFilteredTerm} />
            )}
            <ArtList
        ...
        <video
          ...
          muted
        >
          <source type="video/mp4" src={require('./chizetteArtCrystal.mp4')} />
        </video>
      </AdminProvider>
    </ParallaxProvider>
  );
